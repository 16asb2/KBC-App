...
### Changed
- **The app no longer signs you back in by itself.** Two things conspired to make switching accounts on one device painful. Firebase's `getAuth()` defaults to `browserLocalPersistence`, keeping the session in IndexedDB and restoring it on every launch — so on a shared handset whoever signed in last stayed signed in and the login screen was never seen again. And `new GoogleAuthProvider()` sets no `prompt`, so a browser with exactly one Google account signed in skips the chooser and hands that account straight back: signing out and back in silently returned you to the account you were trying to leave. Auth is now created through `initializeAuth` with `browserSessionPersistence` — signed in while the app is open, reloads included; signed out once it closes — and the provider asks for `prompt: 'select_account'`, so Google's chooser appears every time. Persistence is set at `initializeAuth` rather than by a later `setPersistence()` call because that returns a promise, and until it settles the SDK is still on the default, leaving a window where a stored session can surface through `onAuthStateChanged`. **Trade-off worth knowing:** members on their own phones now sign in on each launch rather than staying signed in indefinitely.
...
- **Calendar: the page scrolls as well as the list.** The tab was pinned to the viewport with only Upcoming Events scrolling inside it, so the month grid was always on screen taking roughly half the display. The page is now taller than the viewport and the list is exactly one screen tall — scrolling the page slides the grid up and away, and once it has gone the list fills the display and scrolls on by itself. Height comes from `svh`, the *small* viewport height, so mobile browser chrome expanding cannot push the bottom of the list out of reach.
