...
### Added
- **A half day, for half the price — and half a punch to pay for it.**
...
### Fixed
- **The grade bar is analog again, and the average is truncated into the band it lands in.** The previous fix snapped a press to one of five steps, which is not what the bar is for: a setter who thinks a problem sits at the soft end of Black should be able to say so, and averaging a dozen such opinions is the point of collecting them. A vote is a position again — wherever on the bar you pressed — and the marker is drawn exactly there.

  What changes is how a set of votes becomes a colour. The grade is now **the band the average falls in**, not the nearest whole index. Several people voting low in Black average to something like 3.27, which is plainly inside the black band — the marker is drawn there — and rounding called it Pink. Truncating calls it Black, which is what the bar was showing all along.

  Rounding was wrong here for a reason worth stating: **the whole numbers are not the grades.** Index 4 is the far right *edge* of the bar, not the middle of Black, so "nearest whole number" asks which band **boundary** the average is closest to — a question nobody was trying to answer. The bar and the count now always agree, and there is a test asserting exactly that across a spread of vote sets: the grade reported is always the band the marker is drawn in.

- **`admin-web/`: the grade bar records where you pressed, not which band you hit.** It dragged smoothly and then stored `Math.floor(frac × 5)`, so every vote landed on its band's own edge — a Pink recorded as `3` sits at 75% of the bar, at the very top of Pink, and a Black recorded as `4` sits at 100%. Averaged with anything else, votes pinned to the edges drag the result upward: two Pinks and a Black came out Black. It stores the position now, the same as the app, so a vote means the same thing in both places.

- **A grade vote is the band you pressed. It was not, and boulders were counted a grade too low.** The grade bar is five equal colour bands, and the two clients disagreed about what pressing one means. `admin-web/` has always written the **band index** — `Math.floor(frac × 5)`, so pressing anywhere in Black is `4`. `web/` wrote a **position on the 0–4 scale** — `(x / width) × 4` — into the same `gradeVotes` map and the same `setterGradeVote` field. Pressing the middle of Black stored `3.6`; pressing just inside it stored `3.2`. Then every count rounded, and `Math.round(3.2)` is `3`.

  So a boulder that a setter and a member had both marked **Black** was counted, filtered, sorted and summarised as **Pink** — while the bar went on drawing its marker at `3.2 / 4` = 80% of the way along, which is inside the Black band. The screen said Black and every number said Pink, and both were reading the same field. It reached the Boulder Summary as a Black boulder on the Yellow Wall that the Black × Yellow cell insisted was not there.

  Three things change, and they are one change: **pressing** the bar now records the band, whole, exactly as the panel always has; the **marker** is drawn at the centre of its band rather than at `value / 4`, which is what made a Pink average look Black; and a **stored vote is read as the band it was pressed in** — `3.4` was a press at 85%, which is Black — so the history means what the people who cast it intended.

  That last one is safe to apply to every vote ever stored because whole numbers are fixed points: `snap(3)` is `3`. Nothing `admin-web/` wrote moves, nothing written from here on moves, and only the skewed positions are put back where they were aimed. Both clients now read votes the same way, so the bar, the boulder list's grade filter and sort, the climb log's established grade, the summary table and the panel's CSV export finally agree with each other.

- **The boulder summary no longer drops boulders out of the grade × wall table.** A boulder whose `locations` is empty — the add form asks for a wall and never insisted on one — or whose wall is spelled some way `LOCATIONS` does not list was counted in its grade row and in the **Boulders** tile, and then in no column at all. On screen that is a grade with boulders in it and not one cell to put them in, which reads as a count that has stopped working rather than as "nobody said where this one is", and there was nothing on the table to tell those two apart.

  Every boulder now lands in exactly one row **and at least one column**. Ones with no wall on record go under a **No wall** column, which only appears when something needs it, and the note under the table says how many there are, names any wall it did not recognise, and says to open the boulder and pick its wall. Wall names are also matched case- and padding-insensitively now, so a row stored as `yellow wall` counts under Yellow Wall instead of vanishing — the two clients both write the exact strings, but a document edited in the Firebase console or arriving from an older import need not.

  A boulder listing the same wall twice is also counted once on that wall rather than twice.
...
- **`admin-web/`: a member ticked ADM in the directory can now actually sign in.** The gate read `users/{uid}` and nothing else — but a member added from this panel gets a document under a *generated* id (both **Add member** and the CSV import write with `users.add()`), and nothing exists at their Firebase uid until they sign in somewhere. So ticking **ADM** on somebody granted them nothing they could use: the panel that granted it met them with *Access Denied*, unless they happened to have opened the member app first and been linked there.
  ...
  **It carries privileges across; it cannot mint them.** The rules re-check `isAdmin`/`isSupervisor` against the record named in `linkedFrom`, whose stored address must match the signed-in token's — and only a record *already* carrying a staff flag is joined at all, so a member who wanders onto the admin URL is turned away without their profile being touched. Onboarding is the member app's job. Matching by **legal name** stays the member app's business too, where the person picks their own record out of a masked list and the rules refuse to carry roles across it; an admin whose record holds an address they no longer use signs in there first, and the Access Denied screen now says so.

  **And a lookup that failed no longer reads as "you are not an admin".** The directory read sat inside a bare `catch {}`, so an outage or a rules change came back as a flat denial — a wrong answer rather than a failure, sending whoever hit it looking in entirely the wrong place. The screen now says what went wrong and offers **Try again**.

- **A session typed into Google Calendar by hand now reads as a session.** The app recognised its own roster format and only that — a `" + "`-separated segment ending in `(super)` — so while sessions are still being written straight into Google Calendar during the transition, "Artur super", "Super - Artur" and "supervised by Bea" all arrived as **special events**: blue on the calendar and in the schedule, not joinable, and not counted by the gym-open banner on Home, which is the part that actually misleads somebody deciding whether to come climb. The word now counts wherever it appears in the title, bracketed or not, in any capitalisation, and "sup", "supervised", "supervisor" and "supervising" count with it. Such an event is pink, labelled **Climb Session**, joinable, and opens the gym on Home; the roster reads the loose marker the same way, so the supervisor is "Artur" and not "Artur super", and joining one rewrites its title into the app's format.

  Two things it deliberately does not do. **Whole words only** — "Soup night", "Last supper", "Superb Owl party" and "Supermarket run" are not sessions. And **a negated marker does not count**: "No supervisor tonight", "Unsupervised open gym" and "Non-super event" stay special, because the one thing a loose match must not do is put "OPEN NOW" on the Home screen for a title saying the gym is unsupervised.

  What it costs: "super" as an adjective — "Ladies Night (super fun)" — now reads as a session, where the old anchored rule excluded it. That anchor is the same one that threw out every hand-typed session above, so it is a straight trade. An event created **in the app** carries `type: 'specialEvent'` and is unaffected however it is titled, which is the way to keep a "super"-titled event blue.

- **A member who is not a supervisor can see their own profile.** They could not, at all. The one card showing your own name, pass, punch count and contact details sat at the top of the **Members** screen — which is behind `RequireRole check={isPrivileged}` and hidden from the nav for everyone else — so the only people who could read their own record were the ones who could already read everybody's. An ordinary member had no way to check how many punches they had left, correct a phone number, or see when their pass runs out.

  It is its own screen now, at `/profile`, reached from the avatar in the header at every size — not a seventh tab, since the bottom bar is already six wide on a phone. It carries **My passes** (the pass and whether an admin has confirmed it, the dates with the days left on them, punches remaining, last visit, anything awaiting confirmation, and both histories — access passes and visits), **Personal information** with the same edit form supervisors use on a member (names, contact, emergency contact, notes for staff), and **Waivers** with what was signed and when. Nothing here writes a pass: `firestore.rules` has always let a member read and update their own `users/{uid}` as long as the write keeps off roles, passes, punches and confirmations, so this needed no rules change — the gap was that the app never offered the screen.

  The duplicate card is gone from **Members**, which is now the directory and nothing else.

- **`admin-web/`: the member directory is the page now, not a table inside a card inside a scrolling page.** Three scrollbars for two axes: the table's own pair, plus the page's, and the table's horizontal one could only be reached by scrolling the page down first — so the bar you needed in order to read the columns to the right was the one you could not see. There is one of each now, both belonging to the area under the nav bar. Scrolling down is the page scrolling: the heading, the buttons and the filters go with it and the table is left the whole window, with its header row stuck to the top. Scrolling sideways moves the table alone — the heading and filters are pinned to the left, because a search box that slides off the screen when you go looking at the right-hand columns is a search box you have to scroll back to reach. The horizontal bar sits on the bottom edge of the screen and stays there. The card is gone with all of it: a border and two insets around a table wider than the monitor bought nothing.

- **`admin-web/`: the PDF exports are gone, and so are the per-tab Drive buttons.** Every tab carried its own ⬇ PDF and green ☁ Drive pair, each one generating a document nobody was reading and filing it in its own corner of Drive; the Members tab's row held nothing else, so it went entirely. **Backup to Drive** in the header is the one button now, and it already wrote the same ground as CSV. The per-tab ⬇ CSV buttons stay. The waiver's *generated* PDF goes with the rest; the link to a **scanned** waiver on a member's record stays, since that is a document somebody uploaded rather than one this page prints. `html2pdf.js` is no longer loaded at all.

- **`admin-web/`: the pinned name column's header was not pinned.** Only the body cells carried `col-name`; the header cell did not, so it scrolled away with the rest of the header row and left the pinned names labelled by whichever column happened to be passing. It is sticky on both axes now and outranks the other header cells, which are sticky at the same depth and come after it in the DOM. The whole column — header included — is tinted light blue, so it reads as the edge of the table rather than as a row that failed to scroll.

- **`admin-web/`: the member directory opens on what changed last.** Newest first, rather than alphabetically: the question on landing there is what has happened since you last looked, and the alphabet is one click away. Records nobody has touched sort to the bottom.

- **`admin-web/`: the access pass end date follows the start date.** It already followed the *pass* — picking Annual filled in twelve months from the start — but moving the start date afterwards left the end date where it was, which is exactly what renewing somebody looks like. Both fields now recompute it, in the edit modal and in the add-member form, and the end date stays editable for the periods that are not simply the pass's length.

- **`admin-web/`: the purchases already recorded as `Active Member` can now be repaired, from their own notes.** Naming the pass on new purchases left every older row still answering "what did they buy?" with a membership state, and the Purchases tab is a history — the old rows are most of it. The pass was never actually lost: the notes line says what was bought (`Purchased: Annual Pass $450 ($38/m) — expires …`), so a **Fix N legacy access types** button reads it back and writes the real pass onto those rows. It appears only while there is something to repair, shows what it will change before changing it, and touches nothing but `accessType` — the notes are the evidence, and they also carry the price, which no pass label does. Rows whose notes name no pass are left alone and reported rather than guessed at, and each one is flagged in the table so it reads as needing attention.

  **And the Sign-In Book will not write another one.** Its **Access Type** dropdown still led with `Active Member`, which is where these rows came from and would have gone on producing them after every repair. The four dated passes replace it, spelled the way the member's own record spells them.

- **Everything on a claimed record now comes across, and the old copy goes.** Membership, punches, dates, waivers, `memberSince`, sign-in history — all of it, with the superseded record removed so nobody is listed twice. `claimsThisRecord` no longer refuses to remove a record carrying a staff flag, which was the whole of the duplication described below.

  **The two staff flags are the exception, and cannot be otherwise.** `firestore.rules` refuses a name-matched write that arrives holding `isAdmin` or `isSupervisor`, and the reason is worth stating plainly: a legal name is public knowledge around a gym, and anyone at all can sign in with a fresh Google account — so a name-matched role would mean a stranger who knows an admin's full legal name becomes an admin over every member's record. They are not discarded either. The claimed record's flags travel as `claimedRoles`, the directory shows a **Roles to review** badge against that member, and opening them pre-ticks the boxes so granting is one Save. Saving clears the flag either way, because declining is also an answer.

- **Claiming a staff record by name produced a duplicate every single time.** Not sometimes — always, and by construction. `firestore.rules` refuses a name-matched write that arrives carrying `isAdmin` or `isSupervisor`, so the copy has to drop them; it then refuses the *delete* of the original, which still has them. The record was joined to the member's account and the old one left standing beside it. Such a record is now shown on the confirm screen but not offered as claimable, saying why: sign in with the address already on it, which the rules can verify and which carries the roles across intact, or ask an admin to link it.

  **And the failed delete said nothing.** It was swallowed into a `console.warn` on the reasoning that the member holds their record either way — which is true, and they also end up listed twice with no sign anything went wrong. That is exactly how this went unnoticed. Removing the old copy is part of the join, not tidying afterwards, so a failure now says so; the join has already succeeded by that point, and saving again finds the profile in place and carries on.

- **The setup form asks who you are before anything else, and the answer is yours to give.** Finding a returning member by email finds the easy cases and misses the ones that matter — people change addresses, and the sheet has whichever one they used years ago. The form now opens on the legal name alone, which is the identity the gym files its records under, looks for every record carrying it, and shows what it found: *"We already have a member on file under that name. Is this you?"* with the address masked to `j•••@gmail.com`. Choose one and it is adopted whole, with everything on it; choose "None of these" and you are a new member.

  The version this replaces guessed, silently, and only when exactly one record carried the name — so two members sharing a legal name matched neither, which reads as "we have never heard of you" and quietly created a third. Who somebody is has an answer and the person signing in is the only one who knows it, so they are asked. The address is masked because whoever reads that screen has proved only that they can type a name.

- **A profile now has two addresses.** `email` is the Google account that signs in; `preferredEmail` is where KBC writes. Signing in with whichever account is on the phone in your hand while being reached somewhere else entirely is the ordinary case here — so adopting a record keeps the address it was filed under, and the setup form lets it be edited rather than silently replacing it with the Google one.

- **The waiver accepted names that were not the member's.** `memberName` fell back from the record's legal name through the Google display name to the email address to the literal string `Unknown`, which turned the one rule the screen enforces into "type whatever the profile happens to be called". It is the legal name on file now and nothing else; a record without one is refused outright rather than signed against a fallback.

- **The waiver could not be signed at all by some members.** The typed name was checked against the record's legal name with only the *typed* side trimmed and neither side folded, so a record carrying `Jane  Smith` or `José` could not be signed for however carefully it was typed. Both sides now fold the way the record matcher folds a name — case, accents, runs of whitespace. The Sign button was also `disabled` on a mismatch, which meant the entire feedback on getting it wrong was a button that did nothing: it is enabled as soon as there is a name to check, and the error says what was expected.

- **The setup form explained itself instead of getting out of the way.** Three sets of headings, a list of missing-field chips and a note about being signed in under the wrong address — the app narrating its own machinery to somebody who only wants to fill in a form. One heading now, and fields that arrive already filled, which says "we have you on file" without being told to.

- **`admin-web/`: a new member's Last Changed was empty.** `addMember` wrote `createdAt`/`createdBy` and no `amendedAt`, and the column reads the later of `amendedAt` and `lastUpdatedAt` — so a record made seconds ago read as one nobody had ever touched. Creating a record is a change to it.

- **`admin-web/`: purchases named a membership state where a pass belongs.** Every membership purchase recorded its access type as `Active Member`, which is not an access pass — so the Purchases tab answered "what did they buy?" with the same two words for a one-month pass and an annual one. Purchases now name the pass, in the same words the member's own record uses. Existing rows keep whatever they were imported with; opening one offers the real passes and keeps the old value as an option rather than silently repointing it. (Those existing rows are repairable in bulk now — see above.)
...
- **An imported purchase carrying a note never appeared on the Purchases tab.** A purchase is a sign-in-book entry whose notes carry `Purchased:` — that marker is the only thing separating a purchase from an ordinary visit, and it is what the tab filters on. A **Notes** column in the import file *replaced* that line rather than extending it, so every row carrying one wrote successfully, counted correctly in the preview, and was then invisible to the only screen that looks for it. The file's note is appended to the marker now, and a row with an empty Notes cell gains no dangling separator.

  **Purchases imported before this fix stay hidden until the file is imported again.** That is safe, and is the repair: an imported purchase's document id is built from the member, the moment and the pass — never the note — so re-running the same file overwrites those records rather than doubling the takings.

- **Members: the two role flags and Last Changed moved to the front of the directory.** They sat at the far right of eighteen columns, which put the three things you scan a directory *for* — who supervises, who administers, whether anyone has touched the record lately — behind a horizontal scroll. They now follow the pinned name column directly, so that whole question is answered without scrolling at all. **Waiver Signed By** is gone as a column: it restated the legal name two columns to its left in every row that had one. It is still shown and still editable in the member's record, where a guardian signing on someone's behalf makes it worth reading.
...
### Added
- **`firestore.rules` is deployed now — `deploy-rules.yml`.** It never was. `firebase.json` names the file, but both hosting workflows use `action-hosting-deploy`, which ships Hosting and only Hosting, so every rules change in this repository's history merged green while production went on enforcing whatever was last pushed by hand. The file read as the source of truth and was not connected to anything, which is the worst shape for an access-control layer to be in — and it is why a member joining their record could not remove the old copy of it: the branch permitting that delete existed only here.

  Runs on `main` alone: a hosting preview channel is its own site, but there is one Firestore and one set of rules for the whole project, so deploying them from a pull request would change what production enforces on the strength of an unreviewed branch. The emulator tests run again in the workflow before the deploy, because rules that lock every member out are not something to find out about from the live app. **The service account may need the Firebase Rules Admin role** — the one `firebase init` creates carries Hosting permissions only.

- **`admin-web/`: the member directory reads like a directory.** It uses the whole window rather than a 1200px column, and the table ends where the window ends — the height was `100svh - 200px`, a guess at everything above it, so the horizontal scrollbar sat off the bottom of the screen whenever an import form was open. It is measured now.

  **A search box and four filters**: name or email, access pass (including "awaiting confirmation"), role, and whether the profile is complete. All of it filters the list already loaded, so nothing here can hide a member the page never fetched.
